...
trap_init_percpu(void)
{
	...
	unsigned int id = thiscpu->cpu_id;
	struct Taskstate *now_ts = &thiscpu->cpu_ts;
	now_ts->ts_esp0 = KSTACKTOP - id * (KSTKSIZE + KSTKGAP);
	now_ts->ts_ss0 = GD_KD;

	extern void sysenter_handler();
	wrmsr(0x174, GD_KT, 0);
	wrmsr(0x175, now_ts->ts_esp0, 0);
	wrmsr(0x176, sysenter_handler, 0);
	// Initialize the TSS slot of the gdt.
	gdt[(GD_TSS0 >> 3) + id] = SEG16(STS_T32A, (uint32_t) now_ts,
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
	//if it's made from kernel, panic it
	...
	if (tf->tf_esp >= UXSTACKTOP - PGSIZE &&
		...
		uptf = (struct UTrapframe *) (tf->tf_esp - utsize - 4);
	} else
