...
mem_init_mp(void)
{
	...
	unsigned int i, pos = KSTACKTOP - KSTKSIZE;
	for (i = 0; i < NCPU; i++, pos -= KSTKSIZE+KSTKGAP)
		boot_map_region(kern_pgdir, pos, KSTKSIZE,
...
//2)
	...
	unsigned int entry = MPENTRY_PADDR / PGSIZE;
	size_t i;
	for (i = 1; i < npages; i++) 
	{
		if (i == entry ||
		    (i >= begin && i < end)) continue;
		pages[i].pp_ref = 0;
