...
sys_env_set_status(envid_t envid, int status)
{
	...
	int ret;
	struct Env *e;
	if ((ret = envid2env(envid, &e, 1)) < 0) return -E_BAD_ENV;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if (envid2env(srcenvid, &src, 1) < 0 ||
	    envid2env(dstenvid, &dst, 1) < 0) return -E_BAD_ENV;
	if (srcva >= (void *) UTOP || ((int) srcva & (PGSIZE-1)) || 
	    dstva >= (void *) UTOP || ((int) dstva & (PGSIZE-1))) return -E_INVAL;
	
	pte_t *pte;
	struct Page *pg = page_lookup(src->env_pgdir, srcva, &pte);
	if (!pg) return -E_INVAL;
	if (!(perm & PTE_U) || !(perm & PTE_P)) return -E_INVAL;
	if ((perm & PTE_W) && !((int) (*pte) & PTE_W)) return -E_INVAL;
	if (page_insert(dst->env_pgdir, pg, dstva, perm)) return -E_NO_MEM;
	return 0;
}
...
sys_page_unmap(envid_t envid, void *va)
{
	...
	struct Env *e;
	if (envid2env(envid, &e, 1) < 0) return -E_BAD_ENV;  
	if (va >= (void*)UTOP || ((int)va & (PGSIZE-1))) return -E_INVAL;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	struct Env *env;
	struct Page *p;
	if ((r = envid2env(envid, &env, 0)) < 0) return -E_BAD_ENV;
	if (!env->env_ipc_recving || env->env_ipc_from != 0) return -E_IPC_NOT_RECV;
	if (srcva < (void*)UTOP)
	{
		if ((int) srcva & (PGSIZE-1) ||
		    (perm & ~PTE_SYSCALL) || ((perm & 5) != 5)) return -E_INVAL;
		pte_t* pte;
		env->env_ipc_perm = 0;
		p = page_lookup(curenv->env_pgdir, srcva, &pte);
		if (p == NULL || ((perm & PTE_W) > 0 && !(*pte & PTE_W) > 0))
			return -E_INVAL;
		if (page_insert(env->env_pgdir, p, env->env_ipc_dstva, perm) < 0)
			return -E_NO_MEM;
	}
...
{
	// LAB 4: Your code here.
	if (dstva < (void*) UTOP && ((int)dstva & (PGSIZE-1))) return -E_INVAL;
	curenv->env_ipc_recving = 1;//record want to receive
