...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	e->env_va_bottom = (uint32_t) ROUNDDOWN(va, PGSIZE);
	uint32_t i;
	for (i = e->env_va_bottom; i < (uint32_t) ROUNDUP(va + len, PGSIZE); i += PGSIZE) 
	{
		struct Page *p = (struct Page *) page_alloc(1);
		if (!p) panic("Memory Out");
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
	...
	for (; ph < eph; ph++)
	{
		...
		int i, sz = ph->p_filesz;
		for (i=0; i<sz; i++) va[i] = binary[ph->p_offset+i];
	}
	...
	region_alloc(e, (void *)(USTACKTOP-PGSIZE), PGSIZE);
     	return;
bad:
	panic("Something wrong in load_icode");
}
