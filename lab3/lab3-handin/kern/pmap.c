...
//1)
	...
	pages[0].pp_ref = 1;
//2)
	unsigned int begin = PGNUM(IOPHYSMEM);
	unsigned int end = PGNUM(PADDR(boot_alloc(0)+1));
	size_t i;
	for (i = 1; i < npages; i++) 
	{
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	{
		if (i >= (void *)ULIM) goto bad;
		p = pgdir_walk(env->env_pgdir, i, 0);
		if (!p || ((*p) & perm) != perm) goto bad;
	}
	return 0;
bad:
