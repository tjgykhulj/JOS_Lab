...
static void
printnum_tjg(void (*putch)(int, void*), void *putdat,
	 unsigned long long num, unsigned base, int *width, int padc)
{
	(*width)--;
	if (num >= base)
		printnum_tjg(putch, putdat, num/base, base, width, padc);
	else if (padc != ' ') {
		int w = *width;
		while (w-- > 0) putch(padc, putdat);
	}
	 ...
	 unsigned long long num, unsigned base, int width, int padc)
{
	...
	// your code here:
	printnum_tjg(putch, putdat, num, base, &width, padc);
	if (padc == ' ')
		while (width-- > 0) putch(' ', putdat);
}
...
=======
        case 'n': {
            ...
            const char *null_error = "error! writing through NULL pointer! (%n argument)\n";
            const char *overflow_error = "warning! The value %n argument pointed to has been overflowed!\n";
	...
	if (!t)
		for (p=null_error; *p; p++) putch(*p, putdat); 
	else
	if (len>127) {
		for (p=overflow_error; *p; p++) putch(*p, putdat);
		*t = -1;
	} else *t = len;
        break;
        }
