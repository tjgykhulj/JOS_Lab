...
//	panic("mem_init: This function is not finished\n");
	...
	boot_map_region(kern_pgdir, KSTACKTOP - KSTKSIZE, 
			...
			PADDR(bootstack), PTE_W);	//user NONE
	...
	boot_map_region(kern_pgdir, KERNBASE,
			ROUNDUP(~KERNBASE, PGSIZE), 0, PTE_W);
...
//3) 4)
	extern char end[];
	struct Page *t = pa2page((physaddr_t)(end - KERNBASE + PGSIZE + npages * sizeof(struct Page)));
	t++;
	cprintf("[page_init] : %x %x\n", page2pa(t), page2pa(&pages[npages-1]));
	t->pp_link = &pages[npages_basemem - 1];//page_free_list;
	chunk_list = NULL;
}
...
page_alloc(int alloc_flags)
{
	...
	if (alloc_flags & ALLOC_ZERO) memset(page2kva(ans), 0, PGSIZE);
	page_free_list = ans->pp_link;
	ans->pp_link = 0;
	return ans;
}
...
{
	cprintf("[page_alloc_npages]\n");

	if (n<=0) return NULL;
	struct Page *ans = page_free_list, *tmp, *pre = 0;
	int i;
	while (ans) 
	{
	  for (tmp = ans, i = 1; i<n; i++, tmp=tmp->pp_link) 
	  {
cprintf("%d %d\n", i, n);
		if (!tmp->pp_link) return NULL;
		if( (page2pa(tmp->pp_link) - page2pa(tmp)) != -PGSIZE )
		{
			pre = tmp; 
			ans = tmp->pp_link; i=-1; break;
		}
	  }
	  if (i == n)
	  {
		if (pre) 
		  pre->pp_link = tmp->pp_link;
		else 
		  page_free_list = tmp->pp_link;
		
		ans = tmp;
		int addr = page2pa(tmp);
		for (i=1; i<n; i++) {
		  addr += PGSIZE;
		  tmp->pp_link = pa2page(addr);
		  tmp = tmp->pp_link;
		}
		if (alloc_flags & ALLOC_ZERO) 
		  memset(page2kva(ans), 0, n*PGSIZE);
		return ans;
	  }
	}
	return NULL;
...
{
	cprintf("[page_free_npages]\n");
	
	if (!pp) return -1;
	struct Page *tmp;
	int i;
	for(tmp = pp, i = 1; i < n; tmp = tmp->pp_link, i++ )
	{
		if(tmp->pp_link == NULL
...
{
	cprintf("[page_realloc_npages]\n");
	struct Page *ans = pp, *temp, *temp2;
	int i;
	if (old_n > new_n) {
		for (i = 1; i < new_n; i++) pp = pp->pp_link;
		temp = pp->pp_link;
		pp->pp_link = NULL;
		page_free_npages(temp, old_n - new_n);
	}
	if (old_n < new_n) {
		temp = chunk_list;
		for (i = 1; i < old_n; i++) pp=pp->pp_link;

		int addr = page2pa(pp);
		while (temp && page2pa(temp) < addr) 
			temp = temp->pp_link;
		
		temp2 = temp;
		for (i = old_n; i < new_n; i++) {
			addr += PGSIZE;
			if (page2pa(temp2) != addr) {
				page_free_npages(ans, old_n);
...
{
	//	cprintf("[pgdir_walk]\n");
	//	cprintf("  PDX(va)=%x, PTX(va)=%x\n", PDX(va), PTX(va));
	pgdir += PDX(va);	//PGADDR(PDX(va), PTX(va), PGOFF(va))
	if (!(*pgdir & PTE_P)) 
	{
//		cprintf("  pgdir have no PTE_P, creating...\n");
		struct Page *page;
		if (create && (page = page_alloc(ALLOC_ZERO))) {
			page->pp_ref++;
			*pgdir = page2pa(page) | PTE_P | PTE_U | PTE_W;
		} else 
			return NULL;
	}
//	cprintf("  now pgdir = %x\n", *pgdir);
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
	...
	cprintf("  old pte = %x, *pte = %x\n", pte, *pte);
	if (!pte) return -E_NO_MEM;
	if (*pte & PTE_P) {
		if (PTE_ADDR(*pte) == page2pa(pp)) //map in the same position
			pp->pp_ref--;
		else 
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	if (pte)
	{
		...
		if (*pte & PTE_P) return pages + PGNUM(*pte);
	}
	return NULL;
}
